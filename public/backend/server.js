...
function authorizeRole(role) {
  return (req, res, next) => {
    ...
    try {
      ...
      req.user = decoded; // infos utilisateur disponibles dans la route
      next();
    } catch (err) {
      return res.status(401).json({ message: 'Token invalide' });
    }
...
app.post('/login', async (req, res) => {
  ...
  } catch (error) {require('dotenv').config({ path: __dirname + '/../../.env' });
console.log('DB_USER:', process.env.DB_USER);
...
// ==============================
app.get('/manage-portal', authorizeRole('admin'), async (req, res) => {
  let connection;
  try {
    connection = await getConnection();
    const [results] = await connection.execute('SELECT * FROM utilisateur');
